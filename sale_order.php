...
if (!empty($_SESSION['cart'])) {
  ...
  if ($result) {
    ...
    foreach ($_SESSION['cart'] as $key => $qty) {
      ...
      $stmt = $pdo->prepare("INSERT INTO sale_order_detail(sale_order_id, product_id, quanity) VALUES(:sid, :pid, :qty)");
      $result =  $stmt->execute(
        array(
          ':sid' => $saleOrderId,
          ':pid' => $id,
      ...
      $stmt = $pdo->prepare("UPDATE products SET quantity=:qty WHERE id=:qid");
      $stmt->execute(
        array(
          ":qty" => $updateQty,
          ":qid" => $id
...
<body>
  ...
  <section class="order_details section_gap">
    <div class="container">
      ...
      <div class="order_details_table"></div>
    </div>
  </section>
