          ...
          <div class="card-body">
            ...
            <table class="table table-bordered">
              ...
              if (empty($_POST['search'])) {
                $smtm = $pdo->prepare('SELECT * FROM posts ORDER BY id DESC');
                $smtm->execute();
                $rawResult = $smtm->fetchALL();
                // ceil formula 
                $totalpages = ceil(count($rawResult) / $noOfRecordsperPage);

                $smtm = $pdo->prepare("SELECT * FROM posts ORDER BY id DESC LIMIT $offset,$noOfRecordsperPage");
                $smtm->execute();
                $result = $smtm->fetchALL();
              } else {
                $searchKey = $_POST["search"];
                $smtm = $pdo->prepare("SELECT * FROM posts WHERE title LIKE '%$searchKey%' ORDER BY id DESC");
                $smtm->execute();
                $rawResult = $smtm->fetchALL();
                // ceil formula 
                $totalpages = ceil(count($rawResult) / $noOfRecordsperPage);

                $smtm = $pdo->prepare("SELECT * FROM posts WHERE title LIKE '%$searchKey%' ORDER BY id DESC LIMIT $offset,$noOfRecordsperPage");
                $smtm->execute();
                $result = $smtm->fetchALL();
              }
                ...
                if ($result) {
                  ...
                  foreach ($result as $value) { ?>
                    <tr>
                      ...
                      <td><?= escape(substr($value['content'], 0, 98)); ?></td>
                      <td>
                        <div class="btn-group">
                          <div class="container">
                          ...
                          <div class="container">
                            <a href="delete.php?id=<?= $value['id'] ?>" type="button" class="btn btn-danger"
