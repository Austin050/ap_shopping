        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    $smtm = $pdo->prepare('SELECT * FROM sale_order_detail WHERE sale_order_id=' . $_GET['id']);
                    $smtm->execute();
                    $rawResult = $smtm->fetchALL();
                    // ceil formula 
                    $totalpages = ceil(count($rawResult) / $noOfRecordsperPage);

                    $smtm = $pdo->prepare("SELECT * FROM sale_order_detail WHERE sale_order_id=" . $_GET['id'] . " LIMIT $offset,$noOfRecordsperPage");
                    $smtm->execute();
