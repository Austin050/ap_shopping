...
if ($_POST) {// back-end validation if block 
  if (empty($_POST['name']) || empty($_POST['description']) || empty($_POST['category'])
    ...
    if ($qtyError == '' && $priceError == '') { // image validation for only the accepted file extensions
      $file = 'images/'.($_FILES['image']['name']);
      $imageType = pathinfo($file,PATHINFO_EXTENSION);

      if ($imageType != 'jpg' && $imageType != 'jpeg' && $imageType != 'png') {
        echo "<script>alert('Image should be jpg,jpeg,png');</script>";
      }else{ //image validation success -> recevives data, write query and add to database
        $name = $_POST['name'];
